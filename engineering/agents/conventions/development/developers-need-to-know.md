...
Likewise, queryability should not be rejected because unrestricted entity exposure is unsafe. Use a governed, mapped and bounded `IQueryable` read contract, and keep writes as explicit commands with explicit authorisation and state rules. See the [IQueryable guidance](../foundations/iqueryable.md).
