...
Do not make physical deletion the default merely because it is easy to describe. Do not make anonymisation a label for hiding a record while keeping a practical route back to a person.
...
Removing the relationship between a record and its owner is often an important step. It is not the complete test.
...
Anonymisation or de-identification is an authorised lifecycle operation. It needs an owner, trigger, policy version, scope, method, risk assessment, result and evidence. It should identify which stores and downstream recipients were covered.
...
There are two common ways to preserve useful history without retaining the person’s identity:
...
The last step is a change in the data's representation and accountable owner inside the system. It is not a legal transfer of ownership to an “anonymous person”. It means that the retained record is no longer managed as information about the original individual because it no longer identifies, singles out or links to that individual. A new stable ID that still joins the person's events would not achieve this; it would remain pseudonymous personal information.
...
- the owner, approval, timestamp, policy version and follow-up review date.
