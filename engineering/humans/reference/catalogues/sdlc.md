...
The phases below are useful because each one asks a different question. They can overlap, repeat and run at different levels. A whole service may be in Operations while a new capability is in Design and another is in Development. A production incident may return an existing capability to Discovery or Definition. A small change may pass through all ten phases quickly; a regulated or long-lived service may need substantial evidence in each.
