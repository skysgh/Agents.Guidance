...
The consumer-side flow is where the experience becomes complete for a person. It coordinates views, focused actions, navigation, client-side validation, loading, progress, errors, retry, cancellation, unsaved work and the return to a meaningful state. The SPA is often the clearest place to discuss how several capabilities are assembled into one user journey.
