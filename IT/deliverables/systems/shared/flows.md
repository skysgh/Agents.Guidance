...
When a word such as Request, Approval, Evidence or Service appears, check its Domain before assuming its meaning. See the [Guidance Glossary](../../../shared/reference/glossary.md) for polysemy, Domain, Capability and Function.

An applicant prepares a request in the client. A service checks it, asks for missing evidence, records a decision and sends an approved outcome to another system. The applicant experiences one journey, but several capabilities and boundaries carry different parts of its meaning. A useful flow makes that journey easier to discuss without making one page or one service responsible for everything.
...
A vertical slice carries one [capability](../../../shared/reference/glossary.md#capability) through the building. A flow connects several [capabilities](../../../shared/reference/glossary.md#capability) across the floors. The flow gives the journey a known route; the slices provide the rooms and services. See [Vertical Slices: Common Shafts](../service/vertical-slices.md) for common slice shapes and [Cross-Cutting Services](../../../development/cross-cutting-services.md) for the shared building systems that support them.
...
Most enterprise applications use several of these shapes. Teams often begin by recognising the closest familiar shape, then adapt it when the actual contracts and responsibilities require something different. A shared flow language reduces the need to invent orchestration terms before the journey itself is understood.
