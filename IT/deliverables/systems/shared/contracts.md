...
The client expects a submitted request to produce a known outcome. The service needs an authorised request, a version and a way to express delay or failure. A provider may accept the instruction later, or may receive it twice after a retry. The contract is the shared place where those expectations meet, before a screen, method or provider payload quietly becomes the authority.
...
The answers do not all need to appear as method signatures. Some belong in the contract's documentation, tests, mapping rules or operational agreement. They must be visible somewhere that the consumer and responsible boundary can use.
...
- [LDM Layers and Contents](../service/layers.md)
...
- [Vertical Slices](../service/vertical-slices.md)
