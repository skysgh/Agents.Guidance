...
Do not implement provider as unrestricted access to all consumer data or all administrative functions.
...
A automated actor that invokes a capability, exchanges information, runs a scheduled operation or performs a controlled lifecycle action. Identify its owner, purpose, credential or key lifecycle, scope, source, destination, rate, retry, idempotency, audit and retirement conditions.

Do not model a service account as a administrator merely because it needs a technical permission. Give it the narrowest machine capability and make its actions distinguishable from person-initiated actions.
