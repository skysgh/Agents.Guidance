...
| 3 | Key Vault and secret resolution | Retrieves protected secrets, credentials, tokens and keys without treating them as ordinary source-controlled configuration. The pipeline and the executing workload use identities to access values only when required; people do not retrieve or copy production secret values. | Can protected material be resolved, classified, rotated and redacted safely, with administration by people separated from value retrieval? |
