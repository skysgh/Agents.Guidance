...
A developer adds one field to a persistence class because the current screen needs it. A report begins using that field, an integration copies it and a later team discovers that the field has become the only explanation of the business relationship. The code still runs, but a storage convenience has started to decide what the organisation means.
...
Read [Logical Layers](../../../shared/reference/catalogues/logical-layers.md), [Logical Building Blocks](../../../shared/reference/catalogues/logical-building-blocks.md), [Contracts](./contracts.md) and [LDM Layers and Contents](../service/layers.md) together when deciding whether a physical property or type belongs in the logical model.
