[Up](../readme.md)

# Palette compatibility route

The canonical client-system Palette now lives in [Client System Guidance](../readme.md). Its [UX Palette](../ux-palette.md), [First Look](../first-look.md) and [Implementation Vocabulary](../implementation-vocabulary.md) provide the detailed routes. This page remains temporarily so older links continue to lead readers to the new system boundary.
