...
A developer implements the prepared responsibility, preserves its contract and raises ambiguity, unsafe assumptions, missing ownership or impossible conditions. Developers do not need to carry the whole architecture in their heads, but they must recognise when code would change a stakeholder promise, boundary, obligation or failure behaviour.

A developer is not automatically the Product Owner, architect, Technical Analyst, tester, operator or maintainer. One person may hold several responsibilities, but the work and evidence for each responsibility should remain visible.

Before making a transient arrangement a property of an enduring entity, check the logical model. A Person is not replaced when a `StudentAt` membership ends, and a Group or School is not defined only by its current members or Location. Raise a design question when the physical schema, API or ORM shape would erase effective dates, authority, history or the ability to translate the concept across systems.
